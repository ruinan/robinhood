...
public class largestGroup {
    ...
    *
    *
    ollow up是找出超过两次同时出现的最多人数，输出他们的人名和出现的时间段
...
//                for (int j = i - 1; j >= 0; j--) {
//                    if (inRoom[j].size() >= 3) {
//                        if (subset(inRoom[i], inRoom[j])) {
//                            // 分别从j 往后，i 往后，找到set 不包含inRoom[index] 的这些人的时候, 记录下两个时刻，然后放到
//                            // 这里要注意，前一个集合的可能是当前集合的子集。某种情况下
//                            // 用set 的另外一个目的是找交集，用retailAll
//                        }
//                    }
//                }
//            }
...
//    }
//
//    private Integer calcTime (String a, String b) {
